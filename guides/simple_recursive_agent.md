...
```elixir
goal = "Find out who won the latest Super Bowl and tell me the score."

{:ok, result} = SimpleRecursiveAgent.run(broker, goal, tools)
...
```elixir
SimpleRecursiveAgent.run(broker, goal, tools, max_iterations: 10)
```
